...
LOG_MODULE_REGISTER(cross_dfu, 3);
 ...
 */
static uint32_t cmd_request_flash_write(uint8_t* p_data, uint16_t length)
{
    //LOG_INF(__func__);
 ...
 * @param length: length of data.
 */
static uint32_t cmd_request_flash_read(uint32_t address, uint16_t length)
{
    //LOG_INF(__func__);
 ...
 */
static uint32_t cmd_request_flash_erase(uint32_t address, uint16_t count)
{
    //LOG_INF(__func__);
 ...
 */
static uint32_t cmd_request_flash_done(void)
{
    //LOG_INF(__func__);
...
static void wk_get_and_write_data_handle(struct k_work* unused)
{
    ...
    if(m_img_size - m_img_offset > IMG_BLOCK_SIZE) //Reading and Writing multiple of kb makes reading and writing faster
...
void startdfuprocess(uint32_t img_size)
{
    ...
    if(err){
        LOG_INF("Err while flash erase cmd %d",err);
    }
}
