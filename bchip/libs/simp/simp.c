...
#define NOISY_RECEIVE                   false           /* Enable if you want to see directly how many Bytes are Received and what */
#define NOISY_MODULE                    false           /* Enable for Module intra Update Logs */

#define CMD_MAX_PACKET_LENGTH           1040            //Should Match the one in Uart_Driver 
...
//Memory Managment
static K_MEM_SLAB_DEFINE(cmd_rx_tx_pool, CMD_MAX_PACKET_LENGTH, 2, 4); /* Main Memory Source for CMD Rx and Tx */
...
//Not In Use
/*
static void cmd_buff_alloc(buffer_t* p_buff)
{
    int err = 0;
    err = k_mem_slab_alloc(&cmd_rx_tx_pool, (void **)&p_buff->p_data, K_NO_WAIT);
	__ASSERT(err == 0, "Failed to alloc slab");
    p_buff->length = 0;
    p_buff->offset = 0;
}
//Not in use
static void cmd_buff_free(buffer_t* p_buff){
   k_mem_slab_free(&cmd_rx_tx_pool, (void **)&(p_buff->p_data));
}
*/
static void reset_buffer(buffer_t* p_buff){
    //Only have to reset length and offset
    //if you need to free buffer use the two functions above
    p_buff->length = 0;
    p_buff->offset = 0;
}
...
static void wk_proc_req_handler(struct k_work* unused)
{
    ...
    err_code = cmd_cb_get(cmd.op_code, &cmd_cb);
    if (err_code == 0) {
        //If Req has User Defined work todo
        if (cmd_cb.proc_req) {
...
static void wk_proc_rsp_handler(struct k_work* unused)
{
    ...
    err_code = cmd_cb_get(cmd.op_code, &cmd_cb);
    if (err_code == 0) {
        //If Rsp has User Defined work todo
        if (cmd_cb.proc_rsp) {
...
static uint16_t cmd_len_get(buffer_t* p_buff)
{
    ...
    if (p_buff->length > CMD_FMT_SIZE_START + CMD_FMT_SIZE_LEN) {
        len = uint16_decode(&p_data[CMD_FMT_OFFSET_LEN]);
...
static int cmd_to_buff(app_cmd_t* p_cmd, buffer_t* p_buff)
{
    ...
    p_packet[CMD_FMT_OFFSET_START] =
        (p_cmd->type == X_CMD_TYPE_RESPONSE) ?
        CMD_FMT_START_RSP :
        CMD_FMT_START_REQ;
...
static void on_uart_rx_ready(uint8_t* b_memory_block, uint16_t length,uint16_t offset)
{
    int err_code;
    uint8_t  req_op_code;

    static bool rx_started; //false init
    static uint16_t cmd_len;

    //Check for which mode
    //TODO switch context between cmd and data stream receive
    if (mode_get(&m_cmd_ctx) == CMD_MODE_HOST) {
    ...
    if (mode_get(&m_cmd_ctx) == CMD_MODE_SLAVE) {
        //if(m_cmd_ctx.state) data rec needs its own logik also maybe rename cmd ctx to simp ctx as its not cmd specific
    ...
    if (!rx_started) {
#if NOISY_MODULE
    #if NOISY_RECEIVE
        LOG_DBG("Initialize Receive of CMD");
    #endif
#endif
        rx_started = true;
        reset_buffer(&m_rx_buff); //Rx Reset
        ...
        cmd_len = 0;
        on_cmd_receive_start(); //Sets State to Receiving and if Req Slave if RSP wont change
    }else{
...
#if NOISY_MODULE
    #if NOISY_RECEIVE
    LOG_DBG("Rec Data: Offset %d Length %d",offset,length);
    #endif
#endif
...
#if NOISY_MODULE
    #if NOISY_RECEIVE
        LOG_HEXDUMP_DBG("RX", m_rx_buff.length ,m_rx_buff.p_data);
    #endif
#endif
 ...
 * @return neg: error
 */
int app_cmd_init(const struct device* p_device)
{
    int err;
    ...
    //Allocate RX Buffer (we dont have to reset this during use it just makes it always have this memory if allocated through slab)
    err = k_mem_slab_alloc(&cmd_rx_tx_pool, (void **)&(m_rx_buff.p_data), K_NO_WAIT);
	__ASSERT(err == 0, "Failed to alloc slab");
    //Tx Buffer also gets allocated here for the Same reason
    err = k_mem_slab_alloc(&cmd_rx_tx_pool, (void **)&(m_tx_buff.p_data), K_NO_WAIT);
	__ASSERT(err == 0, "Failed to alloc slab");

    //Data Streams are realised differently go to uart rx callback to see how it works
